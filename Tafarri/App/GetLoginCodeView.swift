...
class HttpGetLoginCode: ObservableObject {
    ...
    func getLoginCode(user_email: String) {
        ...
        guard let url = URL(string: TafarriApp.app_domain + "/api/v1/user/send-login-code") else { return }

        let body: [String: String] = ["user_email": user_email, "app_type": "ios", "app_version_code": TafarriApp.app_version]
        print(body)
